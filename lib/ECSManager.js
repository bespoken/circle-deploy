...
class ECSManager {
	static async createService(targetGroupArn, taskDefinitionArn) {
		...
		//EC2 launch type is not compatible with awsvpc network configuration
		if (Config.str("launchType") != "FARGATE")
			delete serviceInfo.networkConfiguration;
	...
	static _taskDefinition() {
		let taskDefinitionFile = Config.str("launchType") == "FARGATE" ?
			"../TaskDefinition.fargate.json" :
			"../TaskDefinition.ec2.json";

		taskDefinitionFile = path.join(__dirname, taskDefinitionFile);
		...
		taskDefinitionString = Util.substitute(taskDefinitionString, "containerPort", Config.int("containerPort", 3000));
		taskDefinitionString = Util.substitute(taskDefinitionString, "cpu", Config.str("cpu", 2));
		...
		taskDefinitionString = Util.substitute(taskDefinitionString, "memory", Config.str("memory", 128));
