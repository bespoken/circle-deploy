...
class FargateHelper {
	static async run() {
		...
		for (let i = 3; i < process.argv.length; i += 2) {
			...
			if (key == "env") {
				...
				// Read in an env file that is passed as a single key valiue :-) Useful for Github actions
		...
		if (action === "create") {
			...
			return FargateHelper._run();
		} else if (action === "stop") {
			return FargateHelper._stop();
	...
	static async _run () {
		...
		const taskDefinition = await ECSManager.registerTaskDefinition();
		const taskDefinitionArn = taskDefinition.taskDefinition.taskDefinitionArn;
		return ECSManager.runTask(taskDefinitionArn)
	}
	...
	static _linesToOptions(options, lines) {
		for (const line of lines) {
			...
			const envKey = keyValue[0];
			console.log(`Add Property: ${envKey}:${keyValue[1]}`)
			options.env[envKey] = keyValue[1];
		}
	}
}
