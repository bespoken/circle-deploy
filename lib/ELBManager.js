...
class ELBManager {
	...
	static async createTargetGroup() {
		...
		var params = {
			...
			HealthCheckPort: Config.str("launchType") == "FARGATE" ? Config.str("containerPort") : "traffic-port",
			...
			Protocol: "HTTP",
			TargetType: Config.str("launchType") == "FARGATE" ? "ip" : "instance",
	...
	static async describeTargetGroup(targetGroupName) {
		const client = new AWS.ELBv2();
		...
		return targetGroupsResponse.TargetGroups[0];
	}
}
